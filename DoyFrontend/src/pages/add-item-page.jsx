...
export default function AddItemPage() {
  ...
  useEffect(() => {
    axios.get(`http://localhost:8080/api/item/get-types`)
        .then(response => {
          // Assuming response.data is an array of allergen strings like ["FISTIK", "GLUTEN"]
          if (Array.isArray(response.data)) {
            setAvailableAllergens(response.data);
          } else {
            // Handle case where response.data might be an object with a property containing the array
            // For example, if response.data = { allergens: ["FISTIK"] }
            // This depends on your API structure. For now, assuming direct array.
            console.warn("Fetched allergen types are not in the expected array format:", response.data);
            setAvailableAllergens([]); // Set to empty if format is wrong
          }
        })
        .catch(error => {
          console.error("Error fetching allergen types:", error);
          setErrors(prev => ({ ...prev, allergens: "Alerjen tipleri yüklenemedi." }));
        });
  }, []);
  ...
  const handleAllergenChange = (allergenValue, checked) => {
    ...
    setTouched((prev) => ({ ...prev, allergens: true })); // Mark allergens as touched
    if (validationAttempted) validateForm(); // Re-validate if needed
  };
              ...
              {showSuccess ? (
                  ...
                  >
                    ...
                    >
                      ...
                      <motion.div
                        custom={5} // Adjust custom prop index if other fields changed
                        variants={formFieldVariants}
                        initial="hidden"
                        animate="visible"
                        className="space-y-3"
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-2">
                        {availableAllergens.map((allergenKey) => (
                            ...
                            >
                              ...
                              >
                                {allergenKey} {/* MODIFIED: Directly use allergenKey */}
                              </label>
                            </motion.div>
                        ))}
                        </div>
