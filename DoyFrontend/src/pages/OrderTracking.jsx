            ...
            <div style={{
                ...
                <button
                    ...
                    style={{
                        ...
                        color: darkMode ? '#aaa' : '#888', lineHeight: '1',
                    }}
                ...
                {/* Modal Title */}
                <h3 style={{ textAlign: 'center', marginBottom: '1.5rem', borderBottom: `1px solid ${borderColor}`, paddingBottom: '0.5rem' }}>
                ...
                {/* Close Button (Bottom) */}
                <Button onClick={onClose} style={{ marginTop: '1.5rem', display: 'block', marginLeft: 'auto', marginRight: 'auto' }}>
                    Kapat
                </Button>
            </div>
        </div>
...
export default function OrderTrackingPage() {
    ...
    const initialStatuses = [
        { title: "Onay Bekleyenler", color: "#EECED0", status: OrderStatus.PLACED, buttons: ["Sipariş Detay", "Sipariş Alındı"], canReject: true },
        { title: "Onaylanmış", color: "#E7D3C6", status: OrderStatus.ACCEPTED, buttons: ["Sipariş Detay", "Hazırlanıyor"], canReject: false },
        { title: "Hazırlanıyor", color: "#E7F0D8", status: OrderStatus.ON_PREPARATION, buttons: ["Sipariş Detay", "Hazır"], canReject: false },
        { title: "Hazır Siparişler", color: "#D1E8D4", status: OrderStatus.PREPARED, buttons: ["Sipariş Detay", "Kurye Ata"], canReject: false },
    ];
        ...
        try {
            ...
            if (response.data && Array.isArray(response.data.orderInfoList)) {
                ...
                const displayableStatuses = [
                    ...
                    OrderStatus.ON_PREPARATION, OrderStatus.PREPARED
                ];
                const activeOrders = response.data.orderInfoList.filter(
    ...
    const updateOrderStatus = async (orderId, newStatus, accept = true) => {
        setError(null);
        console.log(`updateOrderStatus: Attempting update for Order ID: ${orderId}, New Status: ${newStatus}, Accept: ${accept}`);
    ...
    const fetchOrderDetails = async (orderId) => {
        ...
        try {
            ...
            console.error(`WorkspaceOrderDetails: Error fetching order details for ${orderId}:`, err.response || err.message || err); // Kept existing log prefix
    ...
    const handleMoveOrder = (order) => {
        ...
        if (currentStatusIndex >= 0 && currentStatusIndex < initialStatuses.length - 1) {
            ...
            console.warn(`handleMoveOrder: Could not find status index or order is in unexpected state. Index: ${currentStatusIndex}, Status: ${order.status}`);
        }
    };
    ...
    const handleCourierAssign = async (courier) => {
        ...
        console.log(`handleCourierAssign: Sending assign request for Courier ID: ${courierId} (${courier.firstName}) to Order ID: ${orderId}`);
        ...
        try {
            ...
            console.error(`handleCourierAssign: Error sending POST request for Courier ID ${courierId}, Order ID ${orderId}:`, err.response || err.message || err);
            // Update UI to show failure for this specific courier
            setCourierRequests(prev => ({ ...prev, [courierId]: "failed" }));
            // Display a general error message to the user
            setError(`Kurye ${courier.firstName} için istek gönderilirken hata oluştu: ${err.response?.data?.message || err.message || 'Bilinmeyen sunucu hatası'}`);
        }
    };
            ...
            <div style={{ padding: "1rem 2rem 2rem 2rem", flexGrow: 1 }}>
                ...
                {error && !loading && (
                    <p style={{
                        ...
                        backgroundColor: '#dc3545', padding: '0.75rem', borderRadius: '8px', margin: '1rem auto', maxWidth: '800px'
                    }}>
                        Hata: {error}
                    </p>
                )}
                    ...
                    <div style={{
                        ...
                        {initialStatuses.map((statusInfo) => (
                            <div key={statusInfo.status} style={{
                                ...
                                gap: "1rem", alignItems: "center", minHeight: "150px"
                            }}>
                                {/* Column Title */}
                                ...
                                {allOrders
                                    ...
                                    .map((order) => (
                                        ...
                                        <div key={order.orderId} style={{
                                            ...
                                            gap: "0.75rem", boxShadow: "0 2px 5px rgba(0,0,0,0.15)"
                                        }}>
                                            {/* Card Content */}
                                            <img src="https://cdn-icons-png.flaticon.com/512/3075/3075977.png" alt="" style={{ width: "45px", height: "45px", objectFit: "contain" }}/>
                                            <div style={{ fontWeight: "bold", fontSize: "0.9em" }}>ID: {order.orderId}</div>
                                            <div style={{ fontSize: "0.85em", textAlign: 'center', color: darkMode ? '#ccc' : '#555', wordBreak: 'break-word' }}>{order.customerName}</div>
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', width: '90%', gap: '0.5rem', marginTop: '0.5rem' }}>
                                                <Button onClick={() => handleShowDetailsClick(order)} size="small">{statusInfo.buttons[0]}</Button>
                                                {statusInfo.buttons[1] && <Button onClick={() => handleMoveOrder(order)} size="small">{statusInfo.buttons[1]}</Button>}
                                                {statusInfo.canReject && <Button onClick={() => handleRejectOrder(order)} style={{ backgroundColor: '#dc3545', color: 'white', border: 'none' }} className="hover:bg-red-700" size="small">Siparişi Reddet</Button>}
                                            </div>
                                        </div>
                                    ))}
                                ...
                                {allOrders.filter(order => order.status === statusInfo.status).length === 0 && (
                                    <p style={{fontSize: '0.85em', color: darkMode ? '#aaa' : '#666', marginTop: '1rem', textAlign: 'center'}}>Bu durumda sipariş yok.</p>
                                )}
                            </div>
                        ))}
                    </div>
