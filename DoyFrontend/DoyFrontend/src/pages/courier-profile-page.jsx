...
export default function CourierProfilePage() {
  ...
  const [courier, setCourier] = useState({
    fullName: "Kuryenin Kullanıcı Adı",
    firstName: "Kuryenin Adı",
    lastName: "Kuryenin Soyadı",
    email: "Kuryenin Email Adresi",
    address: "Adres",
    idNumber: "Kuryenin T.C. Kimlik Numarası",
    vehicleType: "Motorsiklet",
    licensePlate: "Araç Plakası",
    workSchedule: "Tam Zamanlı",
    experience: "0",
    workingDays: {
      Pazartesi: true,
      Salı: true,
  ...
  const handleLogout = () => {
    ...
    // Since we're not using wouter, we'll just log for now
      ...
      >
        ...
        <div className="flex items-center gap-4">
          ...
          <Link href="/auth?tab=register">
            <motion.button
              className={`${darkMode ? "bg-amber-400" : "bg-amber-200"} text-amber-800 rounded-full px-4 py-1 text-xs font-medium`}
              whileHover={{ scale: 1.05, backgroundColor: darkMode ? "#fbbf24" : "#fcd34d" }}
              whileTap={{ scale: 0.95 }}
            >
              KAYIT
            </motion.button>
          ...
          <Link href="/auth?tab=login">
            <motion.button
              className="bg-white text-amber-800 rounded-full px-4 py-1 text-xs font-medium"
      ...
      <div className="flex-grow flex justify-center items-start px-4 pb-8">
        ...
        >
          ...
          <motion.div className="space-y-4 mb-6" variants={itemVariants}>
            ...
            {/* Additional form fields with similar styling */}
            {/* Email field */}
            <motion.div variants={fadeInVariants}>
