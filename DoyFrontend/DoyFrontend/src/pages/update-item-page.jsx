...
export default function UpdateItemPage() {
  ...
  const { id: restaurantId, categoryId, itemId } = params
  ...
  useEffect(() => {
    if (isEditMode && itemId) {
      ...
      axios
        .get(`http://localhost:8080/api/item/get-item/${itemId}`)
        .then((response) => {
          ...
          setFormData({
            ...
            category: getCategoryIdFromType(item.category),
            image: null,
          })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formDataToSend.append("category", reverseCategoryMap[formData.category])
      formDataToSend.append("restaurantId", restaurantId || "1") // Use the restaurant ID from URL params
      ...
      if (isEditMode) {
        ...
        // Create new item
        response = await axios.post("http://localhost:8080/api/item/create", formDataToSend, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        })
      ...
      setLocation(`/restaurants/${restaurantId}/manage`)
    } catch (error) {
      ...
      <main className="container mx-auto max-w-3xl px-4 py-8">
        ...
        >
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={() => setLocation(`/restaurants/${restaurantId}/manage`)}
